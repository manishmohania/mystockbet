...
function manage_volume_indicator(chkbxv, ischkbxchecked) {
  ...
  if (ischkbxchecked) {
    $.get('volume.php?qs='+encodeURIComponent(ts)+'&qtf='+tf+'&qindv='+indv,function (ajxdata, status) {
      ...
      var ind_menu_html = get_volume_ind_menu(k, cn, dn);
      $('#chart-menu-'+k).append(ind_menu_html);
    }); // end ajax
  ...
  var cnum = tsa[2];
    ...
    if (am == 0) {
      ...
      for (var i = c.charts.length-1; i >= 0; --i) {
        ...
        if (!c.charts[i].title.get('text').startsWith('gm-chart')) {
          //remove chart
...
function remove_ema_ds(k, cn, dn, frcie) {
      var emap = [2, 3, 5, 8, 9, 10, 11, 12, 13, 14, 20, 21, 26, 34, 50, 55, 89, 100, 144, 200, 233];
      var c = chart_containers.get('chart-'+k+'-container');
...
function get_volume_ind_menu(k, c, d) {
  // indicator menu
  var h = '<div class="btn-group" role="group" id="chart-menu-k-'+k+'-c-'+c+'-d-'+d+'">' +
  '<button class="btn btn-primary btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">' +
    ' ☰ ' + c +
...
function get_index_adv_dec_menu_html(k, c, d) {
  ...
  var h = '<li class="dropdown-item"><div class="form-check">' +
      '<input class="form-check-input" type="checkbox" name="indicator-chart-ratio-advance-decline-k-'+k+'-c-'+c+'-d-'+d+'" id="indicator-chart-ratio-advance-decline-k-'+k+'-c-'+c+'-d-'+d+'" value="1" onchange="javascript: manage_adv_dec_chart('+k+', \'indicator-chart-ratio-advance-decline-k-'+k+'-c-'+c+'-d-'+d+'\',\'indicator-chart-ratio-advance-decline-k-'+k+'-c-'+c+'-d-'+d+'\',\''+d+'\')" />' +
      '<label class="form-check-label" for="indicator-chart-ratio-advance-decline-k-'+k+'-c-'+c+'-d-'+d+'">Advance/Decline Ratio</label>'+
      '</div></li>';
...
<div class="row">
 ...
 <div class="btn-group" role="group" >
    ...
    <ul class="dropdown-menu"> <li class="dropdown-item">Advance to Decline Ratio</li>
      <li class="dropdown-item"><div class="form-check"  id="layout_1p">
      <input class="form-check-input" type="checkbox" name="gm-chart-ratio-advance-decline-total-market" id="gm-chart-ratio-advance-decline-total-market" value="1" onchange="javascript: manage_adv_dec_chart(get_first_container(), 'gm-chart-ratio-advance-decline-total-market','gm-chart-ratio-advance-decline-total-market','STKTM')" />
      <label class="form-check-label" for="gm-chart-ratio-advance-decline-total-market">Total Market Adv./ Dec. Ratio</label>
      </div></li>
    </ul>
  </div> 
